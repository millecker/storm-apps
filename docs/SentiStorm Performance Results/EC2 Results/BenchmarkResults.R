...
if (is.na(args[2])) {
  stop("MeasurementSeconds argument is missing!")
} else {
  measurementSeconds <- as.numeric(args[2]);
}
...
###############################################################################
csvInputFile <- args[1]
# csvInputFile <- "~/workspace/storm-apps/docs/SentiStorm\ Performance\ Results/EC2\ Results/c3.8xlarge/tuples.csv"
...
# Add tuples / second
measurement.df$TuplesPerSec <- measurement.df$Avg / measurementSeconds
measurement.df
...
if (!is.na(XticksStart)) {
  ...
  minX <- round(min(measurement.df$Nodes))
}
maxX <- round(max(measurement.df$Nodes))
...
ggplot(measurement.df, aes(x=Nodes,y=TuplesPerSec)) + 
